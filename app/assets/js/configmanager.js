...
const launcherDir = runtimeInfo.userDataPath
...
function encryptSecret(value){
    if(typeof value !== 'string' || value.length === 0 || value === 'sry'){
        return value
    }
    const result = ipcRenderer.sendSync('launcher:secure-encrypt', value)
    if(!result?.ok){
        throw new Error(`Unable to protect account token: ${result?.error || 'unknown error'}`)
    }
    if(result.backend === 'basic_text'){
        logger.warn('The operating system does not provide a secure key store for launcher tokens.')
    }
    return SECRET_PREFIX + result.value
}
...
function decryptSecret(value){
    ...
    const result = ipcRenderer.sendSync('launcher:secure-decrypt', value.slice(SECRET_PREFIX.length))
    if(!result?.ok){
        throw new Error(`Unable to unlock account token: ${result?.error || 'unknown error'}`)
    }
    return result.value
}

function protectAuthenticationDatabase(database){
    for(const account of Object.values(database || {})){
        account.accessToken = encryptSecret(account.accessToken)
        if(account.microsoft){
            account.microsoft.access_token = encryptSecret(account.microsoft.access_token)
            account.microsoft.refresh_token = encryptSecret(account.microsoft.refresh_token)
        }
    }
}

function unlockAuthenticationDatabase(targetConfig){
    for(const [uuid, account] of Object.entries(targetConfig.authenticationDatabase || {})){
        try {
            account.accessToken = decryptSecret(account.accessToken)
            if(account.microsoft){
                account.microsoft.access_token = decryptSecret(account.microsoft.access_token)
                account.microsoft.refresh_token = decryptSecret(account.microsoft.refresh_token)
            }
        } catch(error) {
            logger.error(`Stored credentials for ${uuid} can no longer be unlocked. The account must sign in again.`, error)
            delete targetConfig.authenticationDatabase[uuid]
            if(targetConfig.selectedAccount === uuid){
                targetConfig.selectedAccount = null
            }
        }
    }
}
...
exports.save = function(){
    const persisted = clone(config)
    protectAuthenticationDatabase(persisted.authenticationDatabase)
    const temporaryPath = `${configPath}.tmp`
    fs.writeFileSync(temporaryPath, JSON.stringify(persisted, null, 4), { encoding: 'utf8', mode: 0o600 })
    fs.moveSync(temporaryPath, configPath, { overwrite: true })
}
...
exports.load = function(){
    ...
    if(doLoad){
        ...
        try {
            ...
            unlockAuthenticationDatabase(config)
            doValidate = true
        } catch (err){
            logger.error(err)
...
exports.requestFullRepair = function(){
    config.repairRequested = true
}

exports.consumeFullRepairRequest = function(){
    const requested = config.repairRequested === true
    config.repairRequested = false
    return requested
}
...
exports.setLanguage = function(lang){
    ...
    exports.save()
    ipcRenderer.send('launcher:window-action', 'relaunch')
}
