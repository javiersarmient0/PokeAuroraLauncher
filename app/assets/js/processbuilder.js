...
class ProcessBuilder {
    ...
    build(){
        ...
        const redactedArgs = args.map((arg, index) => {
            ...
            return arg === this.authUser.accessToken || previous === '--accessToken' || previous === '--clientId' || previous === '--xuid'
                ? '<redacted>'
                : arg
        })
        logger.debug('Launch arguments prepared:', redactedArgs)
        ...
        child.on('error', error => {
            logger.error('Unable to start the Minecraft process.', error)
        })
    ...
    _constructJVMArguments113(mods, tempNativePath){
        ...
        function changeOption(filePath, lineToReplace, newLine) {
            try {
                let lines = fs.pathExistsSync(filePath)
                    ? fs.readFileSync(filePath, 'utf8').split(/\r?\n/)
                    : []
                const index = lines.findIndex(line => line.startsWith(lineToReplace))
                if(index >= 0){
                    lines[index] = newLine
                } else {
                    lines.push(newLine)
                }
                fs.outputFileSync(filePath, lines.filter(Boolean).join(os.EOL) + os.EOL)
            } catch(error) {
                logger.warn('Unable to synchronize a Minecraft option.', error)
            }
        }
